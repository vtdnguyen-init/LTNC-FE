...
export function DataTable<TData, TValue>({
  ...
  const UseRows = () => {
    // Gọi hàm confirmUse và lưu kết quả vào biến result
    const result = confirmUse();
    console.log(result);
    // Nếu result là true, tức là người dùng nhấn yes
    if (result) {
      // Gọi hàm handleUseRowsSelected để xóa các hàng đã chọn
      handleUseRowsSelected();
    }
        ...
        <button
          ...
          ${
            table.getFilteredSelectedRowModel().rows.length > 0
              ? "border-green-500 hover:bg-green-700 hover:text-white"
              : "border-black dark:border-gray-2"
          }`}
          onClick={UseRows}
        >
          Use {table.getFilteredSelectedRowModel().rows.length}/
